		...
		const message_transport::SimplePublisherPlugin<sensor_msgs::Image,sensor_msgs::CompressedImage>::PublishFn& publish_fn) const
{
  ...
  if (!getParamNode().searchParam("compressed_image_transport_format", format_param) ||
      !getParamNode().getParamCached(format_param, format))
    format = "jpeg";
  if (format == "jpeg") {
    params[0] = CV_IMWRITE_JPEG_QUALITY;
    std::string quality_param;
    if (!getParamNode().searchParam("compressed_image_transport_jpeg_quality", quality_param) ||
        !getParamNode().getParamCached(quality_param, params[1]))
      params[1] = 80; // default: 80% quality
  }
  else if (format == "png") {
    params[0] = CV_IMWRITE_PNG_COMPRESSION;
    std::string level_param;
    if (!getParamNode().searchParam("compressed_image_transport_png_level", level_param) ||
        !getParamNode().getParamCached(level_param, params[1]))
